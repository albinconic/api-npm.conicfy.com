...
// src/api/middlewares/methodNotAllowedMiddleware.js
var methodNotAllowedMiddleware = (c) => {
  return c.text("Method not allowed!", StatusCodes.METHOD_NOT_ALLOWED);
};
var methodNotAllowedMiddleware_default = methodNotAllowedMiddleware;

// src/api/routes/authRoute.js
var authRoutes = new Hono2();
authRoutes.post("/login", login).all(methodNotAllowedMiddleware_default);
authRoutes.post("/register", register).all(methodNotAllowedMiddleware_default);
authRoutes.get("/logout", logout).all(methodNotAllowedMiddleware_default);
var authRoute_default = authRoutes;

// src/api/errors/customAPIError.js
var CustomAPIError = class extends Error {
  constructor(message) {
    super(message);
  }
};
var customAPIError_default = CustomAPIError;

// src/api/errors/badRequestError.js
var BadRequestError = class extends customAPIError_default {
  constructor(message) {
    super(message);
...
// src/api/errors/notFoundError.js
var NotFoundError = class extends customAPIError_default {
  constructor(message) {
    super(message);
...
// src/api/errors/unauthorizedError.js
var UnauthorizedError = class extends customAPIError_default {
  constructor(message) {
    super(message);
    this.statusCode = StatusCodes.NOT_FOUND;
  }
};
var unauthorizedError_default = UnauthorizedError;

// src/api/errors/unauthenticatedError.js
var UnauthenticatedError = class extends customAPIError_default {
  constructor(message) {
    super(message);
    this.statusCode = StatusCodes.NOT_FOUND;
  }
};
var unauthenticatedError_default = UnauthenticatedError;

// src/api/errors/index.js
var CustomAPIErrorNamespace = {
  CustomAPIError: customAPIError_default,
  BadRequestError: badRequestError_default,
  NotFoundError: notFoundError_default,
  UnauthorizedError: unauthorizedError_default,
  UnauthenticatedError: unauthenticatedError_default
};
var errors_default = CustomAPIErrorNamespace;

// src/api/controllers/imageController.js
var getImage = async (c) => {
  const width = c.req.param("width");
  const height = c.req.param("height");
  if (width && !/^\d+$/.test(width)) {
    throw new errors_default.BadRequestError("Width must be a numeric value.", 400);
  }
  if (height && !/^\d+$/.test(height)) {
    throw new errors_default.BadRequestError("Height must be a numeric value.", 400);
  }
  if (!width || !height) {
    throw new errors_default.BadRequestError("Both parameters (width and height) are required!", 400);
  }
  return c.json({ msg: "test" }, StatusCodes.OK);
};

// src/api/routes/imageRoute.js
var imageRoutes = new Hono2();
imageRoutes.get("/get-image/:width?/:height?", getImage, methodNotAllowedMiddleware_default);
var imageRoute_default = imageRoutes;

// src/api/middlewares/notFoundMiddleware.js
var notFoundMiddleware = (c) => {
  return c.text("Route does not exist!", StatusCodes.NOT_FOUND);
};
var notFoundMiddleware_default = notFoundMiddleware;

// src/api/middlewares/errorHandlerMiddleware.js
var errorHandlerMiddleware = async (err, c) => {
  let customError = {
    statusCode: err.statusCode || StatusCodes.INTERNAL_SERVER_ERROR,
    msg: err.message || "Something went wrong!"
  };
  if (err instanceof customAPIError_default) {
...
app.route("/api/v1/auth", authRoute_default);
app.route("/api/v1/image", imageRoute_default);
app.notFound(notFoundMiddleware_default);
app.onError((err, c) => errorHandlerMiddleware_default(err, c));
